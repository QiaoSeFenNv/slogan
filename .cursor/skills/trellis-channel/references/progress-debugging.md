...
trellis channel messages <channel> --raw --kind progress --last 80 \
  | C:\Users\Administrator\AppData\Local\Programs\Python\Launcher\py.exe -3 -c 'import json,sys; [print((json.loads(l).get("detail") or {}).get("text_delta",""), end="") for l in sys.stdin if l.strip()]'
```
