...
def _get_task_status(trellis_dir: Path, input_data: dict) -> str:
    ...
    if active.stale or not task_dir.is_dir():
        return (
            ...
            f"Next-Action: Run `C:\Users\Administrator\AppData\Local\Programs\Python\Launcher\py.exe -3 ./.trellis/scripts/task.py finish` to clear the stale pointer, "
...
def _build_compact_current_state(
    ...
    if get_tasks_dir and iter_active_tasks:
        try:
            ...
            lines.append(
                f"Active tasks: {task_count} total. Use `C:\Users\Administrator\AppData\Local\Programs\Python\Launcher\py.exe -3 ./.trellis/scripts/task.py list --mine` only if needed."
            )
        except Exception:
...
def _build_workflow_overview(workflow_path: Path) -> str:
    ...
    out_lines = [
        ...
        "Full guide: .trellis/workflow.md. Step detail: `C:\Users\Administrator\AppData\Local\Programs\Python\Launcher\py.exe -3 ./.trellis/scripts/get_context.py --mode phase --step <X.Y>`.",
        "",
    ]
